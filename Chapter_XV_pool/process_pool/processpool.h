...
// <<Linux高性能服务器编程>>第十五章 进程池部分
// 半同步/半异步模式，主进程监听连接请求，通过管道通知子进程接受连接，由子进程接管连接
//
...
/** 描述一个子进程的类，m_pid是目标子进程的PID，m_pipefd 是父进程和子进程通信的管道 */
class process
{
public:
    process() : m_pid(-1) {}
...
/** 进程池类，将它定以为模板类是为了代码复用。其模板参数是处理逻辑任务的类 */
template<typename T>
class processpool
{
private:
...
private:
    /** 进程池允许的最大子进程数量 */
    static const in MAX_PROCESS_NUMBER =16;
    /** 每个子进程最多能处理的客户数量 */
...
/** 从epollfd标识的epoll的内核时间表中删除fd上的所有注册事件 */
...
// TODO 复习补充信号相关知识，这一块理解的很不好，还有函数指针和函数签名
...
/**进程池构造函数。参数listenfd是监听socket，它必须在创建进程池之前被创造，否则子进程无法直接引用它。
 * 参数process_nunmber指定进程池中子进程的数量
 **/
template<typename T>
...
void processpool<T>::setup_sig_pipe()
{
    ...
    int ret = socketpair(PF_UNIX, SOCK_STREAN, 0, sig_pipefd);
    assert( ret != -1);
    setnonblocking(m_epollfd);
...
/** 父进程中m_idx值为-1，子进程中m_idx值大于等于0，我们依据此判断接下来要运行的是父进程还是子进程代码 */
template<typename T>
...
void processpool<T>::run_child()
{
    ...
    /** 子进程需要监听管道文件描述符pipefd，因为父进程将通知它来通知子进程accept新连接 */
    ...
    while (!m_stop)
    {
        ...
        for (int i = 0; i < number; ++i)
        {
            ...
            {
                int client = 0;
                /** 从父、子进程之间的管道读取数据，并将结果保存在变量client中。如果读取成功，则表示有新客户链接 */
                ...
                else
                {
                    ...
                    /** 模板类必须实现init方法，以初始化一个客户连接。我们直接使用connfd来索引逻辑处理对象（T类型的对象），以提高程序效率 */
                ...
                else
                {
                    for (int i = 0; i <  ret; ++i)
                    {
                        switch (signals[i])
                        {
                            ...
                            {
                                /**
                                 * 如果父进程接受终止信号，那么就杀死所有子进程，并等待他们全部结束。
                                 * 通知子进程结束更好的方式是向父、子进程之间的通信管道发送特殊数据
                                 * */
